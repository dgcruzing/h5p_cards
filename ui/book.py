...
class Book:
    ...
    def from_flashcards(cls, flashcard_set_title, flashcards):
        structure = {f"Flashcard {i+1}": {} for i in range(len(flashcards))}
        book = cls(flashcard_set_title, structure)
        for i, flashcard in enumerate(flashcards):
            title = f"Flashcard {i+1}"
            content = f"**Question:** {flashcard['question']}\n\n**Answer:** {flashcard['answer']}"
            book.update_content(title, content)
        return book
